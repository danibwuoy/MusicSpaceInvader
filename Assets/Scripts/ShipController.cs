...
public class ShipController : MonoBehaviour
{
    ...
    void HandleTouchInput()
    {
        if (Input.touchCount > 0)
        {
            ...
            if ((touch.position.x < Screen.width / 2) && canShootBomb && (bombCounter > 0))
            {
                canShootBomb = false;
    ...
    void MovePlayer()
    {
        ...
        {
            MovePlayerYAccelerometer();
            MovePlayerXAccelerometer();
        }
    ...
    void MovePlayerYAccelerometer()
    {
        Quaternion rotXAxis = new Quaternion();
        rotXAxis.eulerAngles = new Vector3(0, 0, 0);

        Vector3 temp = new Vector3(0, Input.acceleration.y, 0);
        this.transform.position += temp * ySensitivity * Time.deltaTime * 10.0f;
        // Y - axis
    ...
    void MovePlayerXAccelerometer()
    {
        Quaternion rotYAxis = new Quaternion();
        rotYAxis.eulerAngles = new Vector3(0, 0, 0);
        Vector3 temp = new Vector3(Input.acceleration.x, 0, 0);
        this.transform.position += temp * xSensitivity * Time.deltaTime * 10.0f;
        // X - axis
